...
int main() {
    ...
    printf("SELFTEST: GYRO(%.3f %.3f %.3f) ACCEL(%.3f %.3f %.3f)\n",
           selftest_results[0], selftest_results[1], selftest_results[2],
           selftest_results[3], selftest_results[4], selftest_results[5]);
    ...
    while (1) {
        ...
        sensors_read_all_data(&accel, SENSORS_ACCEL_RANGE_8G, &gyro,
                              SENSORS_GYRO_RANGE_2000, &temp);

        clock_gettime(CLOCK_MONOTONIC, &tend);

        printf(
            "ACCEL X: %7.2f Y: %7.2f Z: %7.2f TOTAL: %7.2f (g/s)\n", accel.x,
            accel.y, accel.z,
            sqrt(accel.x * accel.x + accel.y * accel.y + accel.z * accel.z));

        printf(
            "GYRO  X: %7.1f Y: %7.1f Z: %7.1f TOTAL: %7.1f (degrees/second)\n",
            gyro.x, gyro.y, gyro.z,
            fabs(gyro.x) + fabs(gyro.y) + fabs(gyro.z));

        printf("TEMP   : %7.2f (celsius)      TIME READING: %7.5f (seconds)\n",
               temp, ((double)tend.tv_sec + 1.0e-9 * tend.tv_nsec) -
